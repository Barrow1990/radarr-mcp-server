...
# Radarr's own REST API version this image was built to call — see
# server.py's module docstring for how it's checked against Radarr's
# GET /api discovery endpoint at runtime.
ARG RADARR_API_VERSION=v3
LABEL org.opencontainers.image.source="https://github.com/barrow1990/radarr-mcp-server" \
      org.opencontainers.image.licenses="MIT" \
      io.radarr-mcp.api-version="${RADARR_API_VERSION}"
...
# Liveness only (process up, HTTP serving) — not Radarr connectivity, so a
# transient Radarr outage doesn't get Dockhand/Docker restarting this
# container in a loop. Use GET /ready separately to check Radarr connectivity.
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD ["python", "-c", "\
import os, sys, urllib.request; \
port = os.environ.get('MCP_PORT', '8932'); \
